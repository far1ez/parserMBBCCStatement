...
parser.add_argument('filename', default='statementcc.pdf', nargs='+', help='PDF file(s) of MBB Statement. Accepts wildcards.')
...
def cc_parser(fname):
    ...
    try:
        table = camelot.read_pdf(fname, pages='1', flavor='stream', password=password,
    ...
    if int(pageNumber) > 1:
        table = camelot.read_pdf(fname, pages='2-'+pageNumber, flavor='stream', password=password,
    ...
    (
        pd
        .concat([df, df2])
        .to_csv(fname.replace('pdf', 'csv'), index=False, encoding='utf-8'),
